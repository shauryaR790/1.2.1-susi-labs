...
function sortPaymentsNewestFirst(paymentsPayload) {
    return [...(paymentsPayload?.items || [])].sort((a, b) => (b.created_at || 0) - (a.created_at || 0))
}

function isPaymentCaptured(payment) {
    return Boolean(payment?.id && (payment.status === "captured" || payment.captured === true))
}

function findCapturedPayment(paymentsPayload) {
    const items = sortPaymentsNewestFirst(paymentsPayload)
    return items.find((row) => isPaymentCaptured(row)) || null
}

function findAuthorizedPayment(paymentsPayload) {
    const items = sortPaymentsNewestFirst(paymentsPayload)
    return items.find((row) => row.status === "authorized" && row.id) || null
}

function findSuccessfulPayment(paymentsPayload) {
    return findCapturedPayment(paymentsPayload)
}

function isRazorpayOrderSettled(razorpayOrder) {
    if (!razorpayOrder) return false
    if (razorpayOrder.status === "paid") return true
    const amount = Number(razorpayOrder.amount) || 0
    const paid = Number(razorpayOrder.amount_paid) || 0
